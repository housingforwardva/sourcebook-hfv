...
b11001_vars <- load_table_vars("B11001", current_acs) %>% 
...
## TABLE B25003 - Tenure of Occupied Housing Units -----------------------------
 ...
 b25003_vars <- load_table_vars("B25003", current_acs) %>% 
 ...
 b25004_vars <- load_table_vars("B25004", current_acs) %>% 
...
b25007_vars <- load_table_vars("B25007", current_acs) %>% 
...
b25009_vars <- load_table_vars("B25009", current_acs) %>% 
...
b25010_vars <- load_table_vars("B25010", current_acs) %>% 
...
b11012_vars <- load_table_vars("B11012", current_acs) %>% 
...
b09021_vars <- load_table_vars("B09021", current_acs) %>% 
...
b25042_vars <- load_table_vars("B25042", current_acs) %>% 
...
b25032_vars <- load_table_vars("B25032_", current_acs) %>% 
...
b25127_vars <- load_table_vars("B25127", current_acs) %>% 
...
b25063_vars <- load_table_vars("B25063", current_acs) %>% 
...
b25014_vars <- load_table_vars("B25014_", current_acs) %>% 
...
b17001_vars <- load_table_vars("B17001", current_acs) %>% 
...
b25106_vars <- load_table_vars("B25106", current_acs) %>% 
...
b19049_vars <- load_table_vars("B19049", current_acs) %>% 
...
b25119_vars <- load_table_vars("B25119", current_acs) %>% 
...
b19013_vars <- load_table_vars("B19013", current_acs) %>% 
  ...
  mutate(race = case_when(
    race == "in current_acs Inflation-Adjusted Dollars" ~ "All households",
    TRUE ~ race
  )) %>% 
...
b25064_vars <- load_table_vars("B25064", current_acs) %>% 
  select(variable = name, label = concept)

## TABLE B25031 - Median Gross Rent by Bedrooms --------------------------------

b25031_vars <- load_table_vars("B25031", current_acs) %>% 
...
b25058_vars <- load_table_vars("B25058", current_acs) %>% 
  select(variable = name, label = concept)
...
# Set requested years
requested_years <- 2010:current_acs
survey <- "acs5"
