...
# Setup data - download to temporary file
...
# Download to a temporary file (will be cleaned up automatically)
temp_file <- tempfile(fileext = ".xlsx")
download.file(url, temp_file, mode = "wb")

# Get all sheet names (excluding "Revisions" sheet)
...
standardize_column_names <- function(data, year) {
  ...
  # Create mapping from pre-2014 to post-2013 naming pattern
  if(year < 2014) {
    column_mapping <- c(
...
remove_duplicate_columns <- function(data) {
  ...
  # Clean column names by removing Excel's automatic duplicate suffixes
...
read_sheet_with_year <- function(sheet_name, file_path) {
  # Read the sheet from temp file, skipping the first row and reading as text initially
  data <- read_excel(file_path, sheet = sheet_name, skip = 1, col_types = "text")
  
  # Clean column names
  names(data) <- str_trim(names(data))
  names(data) <- str_replace_all(names(data), "\\s+", " ")
...
cat("Reading and processing all sheets from temporary file...\n")
all_sheets_data <- map(year_sheets, ~read_sheet_with_year(.x, temp_file))
names(all_sheets_data) <- year_sheets

# Combine all sheets
combined_data <- bind_rows(all_sheets_data)
...
# Debug: show all column names after cleaning
cat("All column names after cleaning:\n")
cat(paste(names(combined_data)[1:10], collapse = ", "), "...\n")

# Find the actual CoC column name - check multiple possible patterns
coc_col <- names(combined_data)[str_detect(names(combined_data), "coc|number")][1]

# If still not found, look for the first column (usually the CoC identifier)
if(is.na(coc_col)) {
  coc_col <- names(combined_data)[1]
  cat("No 'coc' column found, using first column:", coc_col, "\n")
} else {
  cat("Found CoC column:", coc_col, "\n")
}

# Convert numeric columns (except CoC and year)
numeric_cols <- combined_data %>%
  select(-all_of(coc_col), -year) %>%
  names()
...
# Filter for Virginia CoCs and select relevant columns
hic_data <- combined_data %>%
  # Select CoC, year, and all bed count columns
  select(
    all_of(coc_col), 
    year, 
    starts_with("total_year_round_beds")) %>% 
      select(co_c_number, year,
             `Emergency Shelter` = total_year_round_beds_es,
             `Transitional Housing` = total_year_round_beds_th,
             `Rapid Rehousing` = total_year_round_beds_rrh,
             `Other Permanent Housing` = total_year_round_beds_oph,
             `Permanent Supportive Housing` = total_year_round_beds_psh,
             `Safe Haven` = total_year_round_beds_sh
             )
...
# Save the data
# write_csv(virginia_data, "data/virginia_hic_data.csv")
# write_rds(virginia_data, "data/rds/va_hic_data.rds")

# Upload to S3 bucket
s3 <- paws::s3()

temp_file <- tempfile(fileext = ".rds")
write_rds(hic_data, temp_file)
s3$put_object(
  Bucket = "hda-data-hub",
  Key = "hud/hic_data.rds",
  Body = temp_file
)
