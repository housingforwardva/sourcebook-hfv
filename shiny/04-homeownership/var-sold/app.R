...
server <- function(input, output, session) {
  ...
  state_title <- reactive({
    paste("Virginia Homes Sold")
  })
  
  msa_title <- reactive({
    paste("Homes Sold", input$msa_select)
  })
  
  locality_title <- reactive({
    paste("Homes Sold", input$locality_select)
  })
  ...
  output$state_plot <- renderGirafe({
    suppressWarnings(create_interactive_plot(create_sold_plot(state_data(), state_title())))
  })
  
  output$msa_plot <- renderGirafe({
    suppressWarnings(create_interactive_plot(create_sold_plot(msa_data(), msa_title())))
  })
  
  output$local_plot <- renderGirafe({
    suppressWarnings(create_interactive_plot(create_sold_plot(locality_data(), locality_title())))
  })
