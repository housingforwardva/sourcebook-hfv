...
server <- function(input, output, session) {
  ...
  plot_title <- reactive({
    ...
    if (geo$type == "msa" && !is.null(geo$msa)) {
      paste("Median Sales Price -", geo$msa)
    } else if (geo$type == "locality" && !is.null(geo$locality)) {
      paste("Median Sales Price -", geo$locality)
    } else {
  ...
  output$plot <- renderGirafe({
    suppressWarnings(create_interactive_plot(create_price_plot(filtered_data(), plot_title())))
  })
